const usePracticeState = ({
    ...
    const [manualRecovery, setManualRecovery] = useState(() => {
        const saved = localStorage.getItem('manualRecovery');
    ...
    useEffect(() => {
        if (manualRecovery) {
            localStorage.setItem('manualRecovery', JSON.stringify(manualRecovery));
        } else {
            localStorage.removeItem('manualRecovery');
        }
    }, [manualRecovery]);
                    ...
                    if (nextFloorNum <= 35) {
                        ...
                        if (nextFloorData) {
                            ...
                            if (nextFloorStartSquare) {
                                ...
                                const messageData = floorMessages?.[nextFloorNum];
                                if (messageData) {
                                    setTimeout(() => {
                                        setFloorInfoModal({
                                            isOpen: true,
                                        ...
                                        if (messageData.highlights && messageData.highlights.length > 0) {
                                            const highlightsMap = messageData.highlights.reduce((acc, highlight) => {
                                                acc[highlight.squareId] = { type: highlight.type, color: highlight.color };
                                                return acc;
                                            }, {});
                                            setHighlightedSquares(highlightsMap);
                                        } else {
                                            setHighlightedSquares({});
                                        }
                                    }, 100);
                                } else {
                                    setTimeout(() => setHighlightedSquares({}), 100);
                                }
    ...
    }, [runState, megidoConditions, showToastMessage, logAction, updateMegidoConditions, setWinStreak, floorClearCounts, setFloorClearCounts, setSelectedSquare, setModalState, setRecoveryModalState, setStatusBuffModalState, setChoiceModalState, updateGuidance, handleConditionRecovery, floorRefs, floorMessages, setFloorInfoModal, setHighlightedSquares]);
    ...
    const handleOpenRecoveryModal = useCallback(() => {
        setRecoveryModalState({
            isOpen: true,
            title: '手動コンディション回復',
            message: '回復させたいスタイルと、回復する人数を入力してください。',
            showRecoveryAmountInput: false,
    ...
    }, [runState.highestFloorReached, handleConditionRecovery, setRecoveryModalState]);
